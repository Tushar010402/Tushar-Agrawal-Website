...
description: "A single expiring Redis key sent 4,000 requests to PostgreSQL at once and spiked our p99 latency to 9 seconds. Here's how cache stampedes happen, how we debugged ours, and the locking + jitter fixes that cut p99 by 80%."
date: "2026-02-10"
author: "Tushar Agrawal"
tags: ["Redis", "Caching", "Performance", "p99 Latency", "Cache Stampede", "Backend Architecture", "PostgreSQL", "Optimization"]
...
If you want the foundational patterns first, my [Redis caching strategies guide](/blog/redis-caching-strategies-complete-guide) covers the basics this post assumes.
...
- **PostgreSQL `pg_stat_activity`** at 9:00 AM showed dozens of identical `SELECT ... GROUP BY` queries all in `active` state — the smoking gun. (This is also how I diagnosed an unrelated outage in [the connection-pooling war story](/blog/database-connection-pooling-performance-guide).)
...
If you are building on Redis, pair this with the fundamentals in [Redis caching strategies](/blog/redis-caching-strategies-complete-guide), and watch your database the way I describe in [database connection pooling](/blog/database-connection-pooling-performance-guide) — stampedes and pool exhaustion are two faces of the same problem.
